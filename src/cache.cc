...
// this is used to bypass the filling of the incoming cache line to the LLC. If the cache is not likely to be used very often or is very large, LLC bypass is beneficial
        ...
        if (upper_level_dcache[fill_cpu] && upper_level_icache[fill_cpu] && block[set][way].valid != 0)
        {
            ...
            rem_pack.fill_level = fill_level >> 1;
            rem_pack.cpu = fill_cpu;
            rem_pack.address = block[set][way].address;
            rem_pack.full_addr = block[set][way].full_addr;
            rem_pack.data = block[set][way].data;
            rem_pack.instr_id = MSHR.entry[mshr_index].instr_id;
            rem_pack.ip = 0; // writeback does not have ip
