    ...
    for file in files:
        for dir in get_file_directory(level):
            ...
            if(key == "cycles"):
                ...
                # if(i == 0):
                if(file == "bfs-3.trace.gz-bimodal-no-no-no-next_line-lru-1core.txt"):
                    bfs_data = np.append(bfs_data, get_data(dir+file,"l1_miss")*100/get_data(dir+file,"l1_access"))
                # elif(i == 1): 
                elif(file == "cc-5.trace.gz-bimodal-no-no-no-next_line-lru-1core.txt"):
                    cc_data = np.append(cc_data, get_data(dir+file,"l2_miss")*100/get_data(dir+file,"l2_access"))
                # elif(i == 2):
                elif(file=="sssp-5.trace.gz-bimodal-no-no-no-next_line-lru-1core.txt"):
                    sssp_data = np.append(sssp_data, get_data(dir+file,"llc_miss")*100/get_data(dir+file,"llc_access"))
